### SAR model with independent normal-gamma prior and beta prior ###
# rm(list=ls())
...
# spatial SAR post. likelihood
beta_prob <- function(rho, a) 1 / beta(a, a) * ((1 + rho)^(a - 1) * (1 - rho)^(a - 1)) / (2^(2 * a - 1))

# beta binomial prior
bbinompdf <- function(x, nsize, a, b) {
  return(base::beta(a + x, b + nsize - x) / base::beta(a, b))
}

bbinompdf2 <- function(x, nsize, a, b, min_k, max_k) {
...
SAR_W1 <- function(Y, X, W_prior, T, niter = 1000, nretain = 250,
                   beta_prior_mean = matrix(0, ncol(X) + 1, 1),
                   beta_prior_var = diag(ncol(X) + 1) * 10,
                   sigma_a = .001, sigma_b = .001,
  ...
  if (!ROW_STANDARDIZED && GRIDDY_GIBBS) {
    stop("No support fro Griddy Gibbs without row-standardization!")
  }
  ...
  for (i in ii_samples) {
    ...
    for (j in jj_samples) {
      ...
      if (prob.delta == 1) {
        ...
        curr.W[i, j] <- rbinom(1, 1, prob.delta)
      }
    }
  }
  ...
  # }
  if (SYMMETRIC) {
    curr.W[upper.tri(curr.W, diag = T)] <- 0
  }
  ...
  curr.sigma <- as.double(crossprod(Y - tX %*% curr.beta)) / (T * n - k)
  curr.gamma <- matrix(0, n, n)
  curr.rho <- 0
  curr.AI <- as.matrix(solve(.sparseDiagonal(n) - curr.rho * curr.w))
  
  ### Gibbs sampling
  curr.A <- .sparseDiagonal(n) - curr.rho * curr.w
  ...
  for (iter in 1:niter) {
    ...
    curr.beta <- b + t(chol(V)) %*% rnorm(k)
    curr.xb <- tX %*% curr.beta
    curr.txb <- matrix(curr.xb, n, T)
    
    # draw sigma
    curr.ESS <- crossprod(Ay - curr.xb)
    curr.sigma <- 1 / rgamma(1, sigma_a + (T * n) / 2, sigma_b + as.double(curr.ESS) / 2)
    
    ## Griddy-Gibbs step for rho
    if (GRIDDY_GIBBS) {
      logdets <- lndetPaceBarry(curr.w, length.out = griddy_n, rmin = rmin, rmax = rmax)[-griddy_n, ]
      wY <- curr.w %*% tY
      # ess.grid1 = sapply(logdets[,2], function(x) sum(dnorm(as.matrix(tY - x*wY),curr.txb,sqrt(curr.sigma),log = T))  )
      ess.grid <- sapply(logdets[, 2], function(x) -sum(((tY - x * wY) - curr.txb)^2) / (2 * curr.sigma))
      den <- T * logdets[, 1] + ess.grid + log(beta_prob(logdets[, 2], rho_pr))
      ...
      accept <- 0
      while (accept != 1) {
        prop.rho <- rnorm(1, curr.rho, rho_scale)
        if (prop.rho < rmax && prop.rho > rmin) {
          accept <- 1
        }
      }
      prop.A <- .sparseDiagonal(n) - prop.rho * curr.w
      prop.logdet <- suppressWarnings(log(det(prop.A)))
      
      if (iter == 1) {
        curr.logdet <- log(det(curr.A))
      }
      post_curr <- T * curr.logdet +
        sum(dnorm(as.matrix(curr.A %*% tY), curr.txb, sqrt(curr.sigma), log = T)) +
        log(beta_prob(curr.rho, rho_pr))
      post_prop <- T * prop.logdet +
        sum(dnorm(as.matrix(prop.A %*% tY), curr.txb, sqrt(curr.sigma), log = T)) +
        log(beta_prob(prop.rho, rho_pr))
      
      acc_prob <- post_prop - post_curr
      if (is.nan(acc_prob) == FALSE) {
        if ((acc_prob) > log(runif(1, 0, 1))) {
          curr.rho <- prop.rho
          curr.A <- prop.A
      ...
      for (jj in jj_samples) {
        if (W_prior[ii, jj] == 0) {
          ...
          if (bb_pr) {
            ...
            bbprior0 <- bbinompdf2(neighb0, n - 1, bbinom_a, bbinom_b, min_k, max_k) * (1 - curr.W_prior[ii, jj])
            ...
            bbprior1 <- bbinompdf2(neighb1, n - 1, bbinom_a, bbinom_b, min_k, max_k) * curr.W_prior[ii, jj]
            bbprior_ <- bbprior1 / (bbprior1 + bbprior0)
          } else {
          ...
          # change 23.02.2022
          #p1 =   bbprior_ * exp(logdet1*T) * dnorm(err1,0,sqrt(curr.sigma))
          #p0 = (1- bbprior_) * exp(logdet0*T) * dnorm(err0,0,sqrt(curr.sigma))
          p1 <- bbprior_ * exp(logdet1 * T) * exp(-err1 / (curr.sigma))
          p0 <- (1 - bbprior_) * exp(logdet0 * T) * exp(-err0 / (curr.sigma))
...
# X <- matrix(rnorm(n*smallk * T),n*T,smallk)
# ALPHA <- 0
# BETA = c(1,-1)
# SIGMA = 1
# BETA_TRUE <- c(ALPHA, BETA)
# Y = kronecker(.sparseDiagonal(T),Ainv) %*% (ALPHA + X %*% BETA + rnorm(n*T, mean = 0,sd = SIGMA))
# 
# INDIRECT = (sum(Ainv) - sum(diag(Ainv))) * BETA / n
# 
# W_prior = matrix(.5,n,n);
# res1 = SAR_W1(Y,X,W_prior,T,niter = 100,nretain = 50,VERBOSE = TRUE,rho_pr = 1.01,
#               SYMMETRIC = TRUE,bb_pr = FALSE,rjct_pr = TRUE,min_k = 1,max_k = 4,sigma_a = 1,sigma_b = 1,
#               rmin = 0,rmax = 1,
#               ROW_STANDARDIZED = TRUE,GRIDDY_GIBBS = TRUE)
#
# res2 = SAR_W1(Y,X,W_prior,T,niter = 50,nretain = 30,VERBOSE = TRUE,rho_pr = 7.5,
#               SYMMETRIC = TRUE,bb_pr = FALSE,rjct_pr = TRUE,min_k = 5,max_k = 25,sigma_a = 1,sigma_b = 1)
#
# W_prior = matrix(.1,n,n);
# res3 = SAR_W1(Y,X,W_prior,T,niter = 50,nretain = 30,VERBOSE = TRUE,rho_pr = 7.5,
#               SYMMETRIC = TRUE,bb_pr = FALSE,rjct_pr = TRUE,min_k = 1,max_k = n/2,sigma_a = 1,sigma_b = 1)
#
# W_prior = matrix(.5,n,n);
# mbar = n/10; bbinom_b = ((n-1) - mbar) / mbar
# res4 = SAR_W1(Y,X,W_prior,T,niter = 50,nretain = 30,VERBOSE = TRUE,bbinom_b = 1,rho_pr = 7.5,
#               SYMMETRIC = TRUE,bb_pr = TRUE,rjct_pr = TRUE,min_k = 1,max_k = n/2,sigma_a = 1,sigma_b = 1)
#
# res5 = SAR_W1(Y,X,W_prior,T,niter = 50,nretain = 30,VERBOSE = TRUE,bbinom_b = 1,rho_pr = 7.5,
#               SYMMETRIC = TRUE,bb_pr = TRUE,rjct_pr = TRUE,min_k = 5,max_k = 25,sigma_a = 1,sigma_b = 1)
#
# mbar = n/10; bbinom_b = ((n-1) - mbar) / mbar
# res6 = SAR_W1(Y,X,W_prior,T,niter = 50,nretain = 30,VERBOSE = TRUE,bbinom_b = mbar,rho_pr = 7.5,
#               SYMMETRIC = TRUE,bb_pr = TRUE,rjct_pr = TRUE,min_k = 1,max_k = n/2,sigma_a = 1,sigma_b = 1)
#
# res7 = SAR_W1(Y,X,W_prior,T,niter = 50,nretain = 30,VERBOSE = TRUE,bbinom_b = mbar,rho_pr = 7.5,
#               SYMMETRIC = TRUE,bb_pr = TRUE,rjct_pr = TRUE,min_k = 5,max_k = 25,sigma_a = 1,sigma_b = 1)
#
#
...
# # res3 = SAR_W1(Y,X,W_prior,T,niter = 30,nretain = 10,VERBOSE = T,SYMMETRIC = FALSE,bb_pr = TRUE,bbinom_b = 5,bbinom_a = .5)
# # res4 = SAR_W1(Y,X,W_prior,T,niter = 30,nretain = 10,VERBOSE = T,SYMMETRIC = FALSE,bb_pr = TRUE,rjct_pr = TRUE,max_k = 10)
# #
# # res5 = SAR_W1(Y,X,W_prior,T,niter = 30,nretain = 10,VERBOSE = T,SYMMETRIC = TRUE,bb_pr = FALSE,rjct_pr = FALSE)
# # res6 = SAR_W1(Y,X,W_prior,T,niter = 30,nretain = 10,VERBOSE = T,SYMMETRIC = TRUE,bb_pr = FALSE,rjct_pr = TRUE,max_k = 10)
# # res7 = SAR_W1(Y,X,W_prior,T,niter = 30,nretain = 10,VERBOSE = T,SYMMETRIC = TRUE,bb_pr = TRUE,rjct_pr = FALSE)
# # res8 = SAR_W1(Y,X,W_prior,T,niter = 30,nretain = 10,VERBOSE = T,SYMMETRIC = TRUE,bb_pr = TRUE,rjct_pr = TRUE,max_k = 10)
# # #
# # plots
# require(plot.matrix)
# plot(as.matrix(W))
