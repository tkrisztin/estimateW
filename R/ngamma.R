...
#'
#' \eqn{Y_t} (\eqn{n \times 1}) collects the \eqn{n} cross-sectional (spatial) observations for time
#' \eqn{t=1,...,T}. \eqn{Z_t} (\eqn{n \times k_2}) is a
...
#'
#' After vertically staking the \eqn{T} cross-sections  \eqn{Y=[Y_1',...,Y_T']'} (\eqn{N \times 1})
#' and \eqn{Z=[Z_1', ..., Z_T']'} (\eqn{N \times k_2}),
#' with \eqn{N=nT}, the final model can be expressed as:
#'
#' \deqn{
#'  Y =  Z \beta_3 + u,
...
#'            Z = dgp_dat$Z, niter = 100, nretain = 50)
sem <- function(Y, tt, W, Z = matrix(1,nrow(Y),1), niter = 200, nretain = 100,
                 rho_prior = rho_priors(),beta_prior = beta_priors(k = ncol(Z)),
                 sigma_prior = sigma_priors()) {
...
#'            Z = dgp_dat$Z, niter = 100, nretain = 50)
sdem <- function(Y, tt, W, X = matrix(0,nrow(Y),0),Z = matrix(1,nrow(Y),1), niter = 200, nretain = 100,
                rho_prior = rho_priors(),beta_prior = beta_priors(k = ncol(X)*2 + ncol(Z)),
                sigma_prior = sigma_priors()) {
  if (ncol(X) == 0 && ncol(Z) == 0) {stop("Error: At least either X or Z matrix have to be provided.")}
  origX = X
  ...
  for (iter in 1:niter) {
    ...
    for (t in 1:tt) {
      ind1 = ((t-1)*n+1):(t*n)
      Ax[ind1,] = as.matrix(sampler_rho$curr_A %*% tX[ind1,])
    }
...
#' A Markov Chain Monte Carlo (MCMC) sampler for the panel spatial SLX model with exogenous spatial weight matrix
#'
#' The sampler uses independent Normal-inverse-Gamma priors for the slope and variance parameters. The function is
#' used as an illustration on using the \code{\link{beta_sampler}} and \code{\link{sigma_sampler}} classes.
#'
#' The considered spatial panel SLX model takes the form:
#'
#' \deqn{
...
#' res = slx(Y = dgp_dat$Y, tt = tt, W = dgp_dat$W, X = dgp_dat$X, Z = dgp_dat$Z,
#'                   niter = 20, nretain = 10)
slx <- function(Y, tt, W, X = matrix(0,nrow(Y),0),Z = matrix(1,nrow(Y),1), niter = 100, nretain = 50,
  ...
  curr_WX = as.matrix(kronecker(Matrix::.sparseDiagonal(tt),W) %*% X)
