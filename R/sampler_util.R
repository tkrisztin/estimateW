...
#' @param rho The value of the spatial autoregressive parameter. If \code{NULL} (default) then no log determinants or inverses of \eqn{I-\rho W} are generated.
#'
...
#' Sample \eqn{W} based on the current state of the the sampler
#'
#' @param tY An \eqn{n x tt} matrix of dependent variables
#' @param curr.txb1 An \eqn{n x tt} matrix of means based on \eqn{X \beta_1}
#' @param curr.txb2 An \eqn{n x tt} matrix of means that will be spatially lagged based on \eqn{X \beta_2}
#' @param curr.sigma The variance parameter
#' @param W_prior Priors for the spatial weight matrix, generated by \code{\link{W_priors}}
#' @param wdraws An object of class \code{wdraws}
#' @param curr.rho The value of the spatial autoregressive parameter. If \code{NULL} (default) then no log determinants or inverses of \eqn{I-\rho W} are generated.
#'
#' @return An object of class \code{wdraws}
#' @export
sample_W = function(tY,
                    curr.txb1 = matrix(0,nrow(tY),ncol(tY)),
                    curr.txb2 = matrix(0,nrow(tY),ncol(tY)),
                    curr.sigma,W_prior,wdraws,curr.rho = NULL) {
  n = nrow(tY)
  tt = ncol(tY)
  ...
  for (ii in ii_samples) {
    ...
    for (jj in jj_samples) {
      if (W_prior$W_prior[ii, jj] == 0) {
        ...
        if (!is.null(curr.rho)) {
          err1 <- sum((A1[ch_elmnt, ] %*% tY - curr.txb1[ch_elmnt, ] - w1[ch_elmnt,] %*% curr.txb2)^2)
          err0 <- sum((A0[ch_elmnt, ] %*% tY - curr.txb1[ch_elmnt, ] - w0[ch_elmnt,] %*% curr.txb2)^2)
        } else {
          err1 = sum( (tY[ch_elmnt,] - w1[ch_elmnt,] %*% curr.txb2 - curr.txb1[ch_elmnt,])^2 )
          err0 = sum( (tY[ch_elmnt,] - w0[ch_elmnt,] %*% curr.txb2 - curr.txb1[ch_elmnt,])^2 )
        }
        adj <- min(err0, err1)
        err1 <- err1 - adj
        err0 <- err0 - adj
        ...
        if (!is.null(curr.rho)) {
          ...
          p1 =   bbprior_ * stats::dnorm(err1,0,sqrt(curr.sigma))
          p0 = (1- bbprior_) * stats::dnorm(err0,0,sqrt(curr.sigma))
        }
